...
def load_graphs(dataset_name, interval):  # 读取图
...
def load_split_index(dataset_name): # 读取分割
    train_idx = torch.load(r"./data/{}/processed_data/train_idx.pt".format(dataset_name)) # [0:8278]
    val_idx = torch.load(r"./data/{}/processed_data/val_idx.pt".format(dataset_name)) # [8278:10643]
    test_idx = torch.load(r"./data/{}/processed_data/test_idx.pt".format(dataset_name))  # [10643:11825]
    return train_idx, val_idx, test_idx


def load_labels(dataset_name):  # 读取标签
    labels = torch.load(r"./data/{}/processed_data/label.pt".format(dataset_name))  # 11826
    return labels


def load_features(dataset_name): # 读取特征
    des_tensor = torch.load('./data/{}/processed_data/des_tensor.pt'.format(dataset_name))        # [229580, 768]
    tweets_tensor = torch.load('./data/{}/processed_data/tweets_tensor.pt'.format(dataset_name))    # [229580, 768]
...
class dataLoader():
    def __init__(self, graphs, input_nodes, seed, batch_size, shuffle, type):
        super().__init__()
        num_neighbors = [300] * 12 if type == 'train' else [-1] * 12
        self.loader_list = [
